...
router = APIRouter(prefix="/ai", tags=["credit intelligence report"])

@router.post("/generate_credit_report", response_class=JSONResponse)
async def generate_credit_report(
    ...
    # --------------------------------------------------------------------
    try:
        generator = CreditReportGenerator()
